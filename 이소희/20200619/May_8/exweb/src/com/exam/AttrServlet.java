...
//자바웹 애플리케이션에서 데이터를 저장할 수 있는 공간(객체)
//1.요청 객체(HttpservletRequest) : 요청마다 1개씩 새로 생성, 요청 처리(응답전송)가 끝나면 소멸
//2.세션 객체(HttpSession) : 사용자(웹브라우저)마다 1개씩 생성, 웹애플리케이션 실행 중 계속 유지
//						-일정시간동안 접속(요청 전송)이 없는 사용자의 세션 객체는 소멸 가능
//3.서블릿컨텍스트 객체(HttpservletContext) : 웹 애플리케이션 전체에 1개만 생성, 웹애플리케이션
//						-모든 사용자가 공유
//(4)PageContext 객체 : JSP 파일 1개 마다 1개 씩 새로 생성, 해당 JSP 실행이 끝나면 소멸
//						-서블릿이 아닌 JSP에서 사용 가능
...
public class AttrServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 요청 객체에 "reqCnt"라는 이름으로 저장된 값을 가져오기
		AtomicInteger rcnt = (AtomicInteger) req.getAttribute("reqCnt");
		if (rcnt == null) {// 요청 객체에 "reqCnt"라는 이름으로 저장한 적이 없는 경우
			rcnt = new AtomicInteger(0); // 0 값을 가진 정수 객체 생성
		}
		rcnt.incrementAndGet(); // 정수 값을 1증가
		req.setAttribute("reqCnt", rcnt); // 요청 객체에 "reqCnt"라는 rcnt 값을 저장
		...
		session.setAttribute("sesCnt", scnt); // 세션 객체에 "sesCnt"라는 scnt 값을 저장
		...
		if (ccnt == null) {// 서블릿 컨텍스트 객체에 "concnt"라는 이름으로 저장한 적이 없는 경우
			ccnt = new AtomicInteger(0); // 0 값을 가진 정수 객체 생성
		}
		ccnt.incrementAndGet(); // 정수 값을 1증가
		context.setAttribute("conCnt", ccnt); // 서블릿 컨텍스트 객체에 "concnt"라는 ccnt 값을 저장
		

		PrintWriter out = resp.getWriter();
		out.println("request Cnt :" + rcnt);
		out.println("session Cnt :" + scnt);
		out.println("context Cnt :" + ccnt);
	}
