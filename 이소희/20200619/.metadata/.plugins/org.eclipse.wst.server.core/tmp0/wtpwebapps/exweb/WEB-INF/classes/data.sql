...
SELECT * FROM bbs;
SELECT bbs_no, bbs_title, bbs_content, bbs_writer, bbs_reg_date, bbs_count FROM bbs;

--시퀀스 생성 (게시판 글 번호를 위한 자동 증가 정수값 생성)
CREATE SEQUENCE seq_bbs_no;

SELECT seq_bbs_no.NEXTVAL FROM DUAL; --시퀀스외 다음 값(증가된 값) 가져오기
SELECT seq_bbs_no.CURRVAL FROM DUAL; --시퀀스외 현재 값 가져오기
